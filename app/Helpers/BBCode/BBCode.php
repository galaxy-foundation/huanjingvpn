...
class BBCode
{
    ...
    const TAG_NAME_FILE      = 'attachment';
    ...
    protected function generateTag(Tag $tag, &$html, Tag $openingTag = null, array $openTags = [])
    {
        ...
        switch ($tag->name) {
            ...
            case self::TAG_NAME_SPOILER:
                ...
                break;
            case self::TAG_NAME_FILE:
                if ($tag->opening) {
                    $code = '<a class="x-cmd" fileindex="'.$tag->property.'"><i class="fa fa-cloud-download"></i> ';
                } else {
                    $code = '</a>';
                }
                break;
