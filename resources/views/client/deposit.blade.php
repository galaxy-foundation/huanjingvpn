...
@section('page', __('client.dep-title'))
...
<div class="text-center deposit mx-auto">
	...
	<h4>@lang('client.dep-success')</h4>
	<h4><span class="text-success"><i class="fa fa-btc fa-fw"></i><span class="vertical-align-middle">{{ App\Helper::btc($data->value,1) }}</span></span></h4>
	<p>@lang('client.dep-sen4')</p>
</div>
@else
	<div id="invoice">
		<div class="content-block">
			<div class="inner">
				<div>
					...
					<div>
						<h4 class="seller-info">@lang('app.title')</h4>
						<p class="text-muted">@lang('client.dep-invoice',['num'=>$order->id])</p>
					</div>
					<div style="">
						<div style="display: flex;">
							<div style="flex: 1 1 0%;">
								<div class="text-right">
								<h5>{{ App\Helper::btc($order->btc) }} BTC</h5>
								<span class="text-muted">{{ $order->price }} USD</span>
								</div>
							</div>
					...
					<div class="payment-info" id="payment-unpaid">
						<div class="invoice-progress-bar">
							...
							<div class="text-center h3" id="payment-countdown" expired="{{ $data->expired_at }}" now="{{ gmdate("Y-m-d H:i:s") }}">12:00:00</div>
						</div>
						<div class="text-center" id="invoice-qr-code"></div>
						<div class="text-center" id="payment-address">
							<div class="send-amount">
								<div><i class="fa fa-cog fa-spin"></i> @lang('client.dep-send-amount-note')</div>
							</div>
							<div class="form-inline">
								<div>
									...
									<input class="ant-input" id="invoice-amount-input" readonly="" type="text" value="{{ App\Helper::btc($order->btc) }}">
									<span class="ant-input-suffix">
										<div class="ant-btn-group">
											<button id="copy-amount" type="button" class="ant-btn ant-input-search-button ant-btn-icon-only" title="@lang('client.dep-copy')">
								...
								<div>
									<span class="address-input ant-input-affix-wrapper">
										<span class="ant-input-prefix">
											<a href="bitcoin:{{ $data->address }}?amount={{ App\Helper::btc($order->btc) }}" class="ant-btn ant-btn-primary ant-btn-icon-only ant-btn-background-ghost">
										...
										<input class="ant-input" id="invoice-address-input" class="text-monospace" readonly="" type="text" value="{{ $data->address }}">
										<span class="ant-input-suffix">
											<div class="ant-btn-group">
												<button id="copy-address" type="button" class="ant-btn ant-input-search-button ant-btn-icon-only" title="@lang('client.dep-copy')">
													<i class="anticon anticon-copy">
													<svg viewBox="64 64 896 896" class="" data-icon="copy" width="1em" height="1em" fill="currentColor" aria-hidden="true">
														<path d="M832 64H296c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8h496v688c0 4.4 3.6 8 8 8h56c4.4 0 8-3.6 8-8V96c0-17.7-14.3-32-32-32zM704 192H192c-17.7 0-32 14.3-32 32v530.7c0 8.5 3.4 16.6 9.4 22.6l173.3 173.3c2.2 2.2 4.7 4 7.4 5.5v1.9h4.2c3.5 1.3 7.2 2 11 2H704c17.7 0 32-14.3 32-32V224c0-17.7-14.3-32-32-32zM350 856.2L263.9 770H350v86.2zM664 888H414V746c0-22.1-17.9-40-40-40H232V264h432v624z"></path>
													</svg>
									...
									<div class="toast" style="position:absolute">
											@lang('client.dep-copied')
									</div>
								</div>
							</div>
						</div>
						<div class="invoice-notes">
							<div class="invoice-notes-item">@lang('client.dep-note1')</div>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="order-footer-details">
			<div>@lang('client.dep-created', ['time'=>$data->created_at])</div>
		</div>
		<div class="mt-10 buttons" style="margin: 8px 0px;">
			<div class="clearfix">
				<div class="pull-left">
					<span class="back-link">
					<button type="button" class="btn btn-outline-danger">@lang('client.dep-cancel')</button>
					</span>
				</div>
	...
	<!--
	<div id="qrcode"></div>
	<div class="mt-2"><span id="address" class="text-center text-monospace">{{ $data->address }}</span></div>
	<h4 class="mt-4"><i class="fa fa-cog fa-spin"></i> @lang('client.dep-wait')</h4>
	<p>@lang('client.dep-sen2')</p>
